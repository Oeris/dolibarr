...
if ( $_GET['dolibarrkey'] != $conf->global->WEBSERVICES_KEY)
{
  accessforbidden('Value provided into dolibarrkey entry field does not match security key defined in Webservice module setup');
  return false;
}

$user=new User($db);
$result=$user->fetch('',$_GET['login'],'',0);
if ($result < 0)
{
    accessforbidden('A technical error occurred during fetch of user');
    return false;
}
else if ($result == 0)
{
    accessforbidden('Bad value for login or password');
    return false;
}
$user->getrights();
// Authentication mode
...
//print '$accessallowed='.$accessallowed.'<br>';
// Security:
