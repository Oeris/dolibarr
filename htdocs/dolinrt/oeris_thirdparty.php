...
/**
 *       \file       htdocs/dolinrt/oeris_thirdparty.php
...
$thirdparty_fields= array(
      ...
      'typent' => array('name'=>'typent','type'=>'xsd:int'));
...
function createThirdParty($authentication,$thirdparty)
{
    ...
    $result=$company->fetch(0, $thirdparty['ref']);

		if ($company->id > 0)
		{
      $objectresp=array('result'=>array('result_code'=>'OK', 'result_label'=>''),'id'=>$company->id,'ref'=>$company->ref);
    }
    else
    {
      if (! $error)
      {
          ...
          $newobject->phone= ($thirdparty['phone'] == 'undefined') ? '' : $thirdparty['phone'];
...
function updateThirdParty($authentication,$thirdparty)
{
	...
	$fuser=check_authentication($authentication,$error,$errorcode,$errorlabel);
	// Check parameters
	if (empty($thirdparty['id']) && empty($thirdparty['ref']))	{
		$error++; $errorcode='KO'; $errorlabel="Thirdparty id or ref is mandatory.";
	}
	...
	if (! $error)
	{
		...
		$result=$object->fetch($thirdparty['id'],$thirdparty['ref']);

		if (!empty($object->id)) {

			$objectfound=true;

			$object->ref                           =       empty($thirdparty['ref']) ? $object->ref : $thirdparty['ref'];
			$object->name                          =       empty($thirdparty['ref']) ? $object->name : $thirdparty['ref'];
			$object->ref_ext                       =       empty($thirdparty['ref_ext']) ? $object->ref_ext : $thirdparty['ref_ext'];
			$object->status                        =       empty($thirdparty['status']) ? $object->status : $thirdparty['status'];
			$object->client                        =       empty($thirdparty['client']) ? $object->client : $thirdparty['client'];
			$object->fournisseur                   =       empty($thirdparty['supplier']) ? $object->fournisseur : $thirdparty['supplier'];
			$object->code_client                   =       empty($thirdparty['customer_code']) ? $object->code_client : $thirdparty['customer_code'];
			$object->code_fournisseur              =       empty($thirdparty['supplier_code']) ? $object->code_fournisseur : $thirdparty['supplier_code'];
			$object->code_compta                   =       empty($thirdparty['customer_code_accountancy']) ? $object->code_compta : $thirdparty['customer_code_accountancy'];
			$object->code_compta_fournisseur       =       empty($thirdparty['supplier_code_accountancy']) ? $object->code_compta_fournisseur : $thirdparty['supplier_code_accountancy'];
			$object->date_creation                 =       $now;
			$object->note_private                  =       empty($thirdparty['note_private']) ? $object->note_private : $thirdparty['note_private'];
			$object->note_public                   =       empty($thirdparty['note_public']) ? $object->note_public : $thirdparty['note_public'];
			$object->address                       =       empty($thirdparty['address']) ? $object->address : $thirdparty['address'];
			$object->zip                           =       empty($thirdparty['zip']) ? $object->zip : $thirdparty['zip'];
			$object->town                          =       empty($thirdparty['town']) ? $object->town : $thirdparty['town'];

			$object->country_id                    =       empty($thirdparty['country_id']) ? $object->country_id : $thirdparty['country_id'];
			if ($thirdparty['country_code']) $object->country_id=getCountry($thirdparty['country_code'],3);
			$object->province_id                   =       empty($thirdparty['province_id']) ? $object->province_id : $thirdparty['province_id'];
			//if ($thirdparty['province_code']) $newobject->province_code=getCountry($thirdparty['province_code'],3);

			$object->phone                         =       empty($thirdparty['phone']) ? $object->phone : $thirdparty['phone'];
			$object->fax                           =       empty($thirdparty['fax']) ? $object->fax : $thirdparty['fax'];
			$object->email                         =       empty($thirdparty['email']) ? $object->email : $thirdparty['email'];
			$object->url                           =       empty($thirdparty['url']) ? $object->url : $thirdparty['url'];
      ...
      $object->typent_id                     =       empty($thirdparty['typent']) ? $object->typent_id : $thirdparty['typent'];

			$object->capital                       =       empty($thirdparty['capital']) ? $object->capital : $thirdparty['capital'];

			$object->barcode                       =       empty($thirdparty['barcode']) ? $object->barcode : $thirdparty['barcode'];
			$object->tva_assuj                     =       empty($thirdparty['vat_used']) ? $object->tva_assuj : $thirdparty['vat_used'];
			$object->tva_intra                     =       empty($thirdparty['vat_number']) ? $object->tva_intra : $thirdparty['vat_number'];

			$object->canvas                        =       empty($thirdparty['canvas']) ? $object->canvas : $thirdparty['canvas'];
			...
			$result=$object->update($object->id,$fuser);
			if ($result <= 0) {
				$error++;
			}
		}
