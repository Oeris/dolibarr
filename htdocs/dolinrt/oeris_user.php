...
/**
 *       \file       htdocs/dolinrt/oeris_user.php
...
$server->configureWSDL('WebServicesOerisUser',$ns);
...
function createUserFromThirdparty($authentication,$thirdpartywithuser)
{
	...
	if (! $error)
	{
      ...
      $sql.= " AND sp.email='".$db->escape($thirdpartywithuser['email'])."'";
			$sql.= $db->plimit(1);
			...
			if ($resql)
			{
				...
				{
					$error++;
					$errorcode='ALREADY_EXIST'; $errorlabel='Object not create : company or contact exists '.$thirdpartywithuser['email'].'. '.$sql;
				}
				else
				{
					//$db->begin();

					$result=$thirdparty->fetch(0, $thirdpartywithuser['name_thirdparty']);
					...
					if ($thirdparty->id > 0)
					{
						...
						*/
						...
						$contact->email = $thirdpartywithuser['email'];
						$contact->phone_pro = $thirdparty->phone;
						$contact->phone_mobile = ($thirdpartywithuser['phone_mobile'] == 'undefined') ? '' : $thirdpartywithuser['phone_mobile'];
            $contact->phone_perso = ($thirdpartywithuser['phone'] == 'undefined') ? '' : $thirdpartywithuser['phone'];
